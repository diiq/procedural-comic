...
(defun subdivide-vertical-proportional (segments panel)
  (if (<= (length segments) 1)
      ...
      (let ((total (reduce #'+ segments))
        ...
        (maybe-offset-point (loop for part in segments
...
(defmethod shrink-panel (gutter (panel circle-panel))
  (make-circle-panel :center (circle-panel-center panel) :radius (- (circle-panel-radius panel) gutter)))
  ...
  (with-options-for 'offset
    ...
    (if (pick-a 'offset)
        (let* ((panel (car panelset))
               ...
               (new-point (+ old-point (* left-span (/ (pick-one '(-1 1)) (+ 5 (random 3))))))) ; bug here: sometimes offset 0
...
(defun maybe-subdivide-one (panelset)
  (with-options-for 'subdivide
    ...
    (cond ((equal (chosen-value 'horizontal-divisions) 3)
           (rarely 'subdivide 't)))
    (if (pick-a 'subdivide)
  ...
  (with-options-for 'focal-point
    (with-options-for 'focal-point-horizontal
      (with-options-for 'focal-point-vertical
        ...
        (sometimes 'focal-point-horizontal 1/3)
        (sometimes 'focal-point-horizontal 2/3)
        (sometimes 'focal-point-horizontal 1/2)
        (rarely 'focal-point-horizontal (random 1.0))
        (sometimes 'focal-point-vertical 2/3)
        (sometimes 'focal-point-vertical 2/3)
        (sometimes 'focal-point-vertical 2/3)
        (sometimes 'focal-point-vertical 1/2)
        (sometimes 'focal-point-vertical 1/2)
        (sometimes 'focal-point-vertical 1/3)
        ...
        (if (pick-a 'focal-point)
            (let* ((vertical (pick-a 'focal-point-vertical))
                   ...
                   (left-point (+ (panel-bottom-left panel) (* vertical (- (panel-top-left panel) (panel-bottom-left panel)))))
                   (right-point (+ (panel-bottom-right panel) (* vertical (- (panel-top-right panel) (panel-bottom-right panel)))))
...
(defun maybe-add-inset-circle (panelset)
  (with-options-for 'add-circle
    ...
    (if (> (length panelset) 1)
        (rarely 'add-circle nil)
        (rarely 'add-circle 't))
      ...
      (maybe-add-focal-points
       (shrink-panels
        ...
        (maybe-set-one-borderless
         (mapcan (lambda (x) (maybe-add-inset-circle (maybe-subdivide-one (subdivide-horizontal-proportional (horizontal-divisions) x))))
...
(defun division-size ()
  (with-options-for 'division-size
    (sometimes 'division-size 5)
    (sometimes 'division-size 6)
    (sometimes 'division-size 7)
    (sometimes 'division-size 8)
    (sometimes 'division-size 9)
    (sometimes 'division-size 10)
    (pick-a 'division-size)))

(defun horizontal-divisions ()
  (with-options-for 'horizontal-divisions
    (sometimes 'horizontal-divisions 1)
    (sometimes 'horizontal-divisions 2)
    (sometimes 'horizontal-divisions 3)
    (rarely 'horizontal-divisions 4)
    (rarely 'horizontal-divisions 5)
    (if (equal (chosen-value 'vertical-divisions) 1)
...
(defun vertical-divisions ()
  (with-options-for 'vertical-divisions
    (almost-always 'vertical-divisions 2)
    (almost-always 'vertical-divisions 3)
    (almost-always 'vertical-divisions 3)
    (sometimes 'vertical-divisions 1)
    (rarely 'vertical-divisions 4)
    (rarely 'vertical-divisions 5)
